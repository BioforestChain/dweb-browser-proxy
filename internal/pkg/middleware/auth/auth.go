...
func JWTAuth(r *ghttp.Request) {
	...
	if len(token) == 0 {
		r.Response.WriteJson(middleware.Response{
			Code:    error2.TokenCannotBeNull,
			Message: error2.Err.GetErrorMessage(error2.TokenCannotBeNull),
			Data:    nil,
		})
		r.Exit()
		return
	...
	if err != nil {
		r.Response.WriteJson(middleware.Response{
			Code:    error2.TokenIsInvalid,
			Message: error2.Err.GetErrorMessage(error2.TokenIsInvalid),
			Data:    nil,
		})
		r.Exit()
		return
...
//	@return int64
//	@return error
func (s *Auth) GenToken(userID uint32, deviceIdentification string) (string, string, int64, error) {
...
//	@return error
func (s *Auth) ParseToken(tokenString string) (*entity.MyClaims, error) {
	// 解析token
...
//	@return error
func (s *Auth) RefreshToken(accessToken, refreshToken string) (*v1.ClientUserRefreshTokenRes, error) {
	// refresh token无效直接返回
