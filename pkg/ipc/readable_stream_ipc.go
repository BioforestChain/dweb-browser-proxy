...
import (
	...
	"errors"
	"fmt"
	helper2 "github.com/BioforestChain/dweb-browser-proxy/pkg/ipc/helper"
...
func (rsi *ReadableStreamIPC) bindInputStream() (err error) {
	...
	for data := range readInputStream(rsi.inputStream) {
		if len(data) == 4 || len(data) == 5 {
			switch {
			case helper2.BytesEqual(data, "pong"):
				return nil
			case helper2.BytesEqual(data, "close"):
				rsi.Close()
				return nil
			case helper2.BytesEqual(data, "ping"):
...
func (rsi *ReadableStreamIPC) postMessage(ctx context.Context, msg any) (err error) {
	...
	default:
		...
		chunk := helper2.FormatIPCData(msgData)
...
func (rsi *ReadableStreamIPC) EnqueueToInputStream(data []byte) error {
	ipcData := helper2.FormatIPCData(data)
...
func (rsi *ReadableStreamIPC) encode(msg string) []byte {
	...
	return helper2.FormatIPCData([]byte(msg))
}
...
func readInputStream(stream *ReadableStream) <-chan []byte {
	...
	go func() {
		...
		for {
			...
			bodySize := helper2.GetBodySize(header)
			body, err := b.read(bodySize)
			if err != nil {
				break
			}

			helper2.PutBuffer(header)
